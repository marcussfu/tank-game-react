...
const makePlayer = (overrides: Partial<PlayerEntity> = {}): PlayerEntity => ({
    ...
    lives: 3,
    active: true,
    spawn: { position: [780, 20], direction: 'NORTH' },
    ...overrides,
});

const farAwayPlayers = [makePlayer()];

describe('findNextStep', () => {
    it('returns null for an empty target list', () => {
        expect(findNextStep(openTiles, [100, 100], [], [], farAwayPlayers, 1)).toBeNull();
    });

    it('returns null when start is already on a target cell', () => {
        expect(findNextStep(openTiles, [100, 100], [[100, 100]], [], farAwayPlayers, 1)).toBeNull();
    });

    it('returns the direction of a directly adjacent target', () => {
        const targets: Position[] = [[100, 120]]; // one cell south
        expect(findNextStep(openTiles, [100, 100], targets, [], farAwayPlayers, 1)).toBe('SOUTH');
    });
    ...
    it('picks the only viable first step around a local obstruction, regardless of the target beyond it', () => {
        ...
        expect(findNextStep(tiles, [100, 100], targets, [], farAwayPlayers, 1)).toBe('EAST');
    });
    ...
    it('returns null when every target is unreachable (fully boxed in)', () => {
        ...
        expect(findNextStep(tiles, [100, 100], targets, [], farAwayPlayers, 1)).toBeNull();
    });
    ...
    it('routes around another tank occupying the direct path', () => {
        ...
        const step = findNextStep(openTiles, [100, 100], targets, [blockerTank], farAwayPlayers, 1);
    ...
    it('does not treat the searching tank itself as an obstacle', () => {
        ...
        expect(findNextStep(openTiles, [100, 100], targets, [self], farAwayPlayers, 1)).toBe('SOUTH');
    });
