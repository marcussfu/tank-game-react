...
import hudReducer, { setTimeRemaining, setEnemiesRemaining, setLevel, setLives, setScore, resetHud } from './hudSlice';
...
describe('hudSlice', () => {
    ...
    it('setTimeRemaining overwrites timeRemainingSec', () => {
        const state: HudState = { timeRemainingSec: 100, enemiesRemaining: 2, levelIndex: 0, totalLevels: 2, lives: 3, livesP2: null, score: 0 };
    ...
    it('setLives updates player 2 lives for playerId 1', () => {
        const state: HudState = { timeRemainingSec: 100, enemiesRemaining: 2, levelIndex: 0, totalLevels: 2, lives: 3, livesP2: 3, score: 0 };
        expect(hudReducer(state, setLives({ playerId: 1, lives: 1 }))).toEqual({ ...state, livesP2: 1 });
    });

    it('setScore overwrites score', () => {
        const state: HudState = { timeRemainingSec: 100, enemiesRemaining: 2, levelIndex: 0, totalLevels: 2, lives: 3, livesP2: null, score: 0 };
        expect(hudReducer(state, setScore(1500))).toEqual({ ...state, score: 1500 });
    });

    it('resetHud restores the initial values regardless of current state', () => {
        const state: HudState = { timeRemainingSec: 3, enemiesRemaining: 9, levelIndex: 1, totalLevels: 2, lives: 0, livesP2: 0, score: 0 };
